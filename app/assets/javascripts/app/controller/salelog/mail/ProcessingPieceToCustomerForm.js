/**
 * 拆开后单独加载“新增/修改寄加工件(往客户)”视图用的controller
 */
Ext.define('EIM.controller.salelog.mail.ProcessingPieceToCustomerForm', {
...
//    }],
    ...
    init: function() {
        ...
        me.control({
            'mail_processing_piece_to_customer_form combo[name=customer_id]': {
                render: this.loadCustomer,
                select: this.enableExpressButton
            },
            'mail_processing_piece_to_customer_form combo[name=express_id]': {
                select: this.enableExpressButton
            },
            'mail_processing_piece_to_customer_form combo[name=our_company_id]': {
                select: this.enableExpressButton
            },
            'mail_processing_piece_to_customer_form print_express_sheet_button': {
                click: this.printExpressSheet
            },
            'mail_processing_piece_to_customer_form button[action=save]': {
                click: this.validate
            },
            'mail_processing_piece_to_customer_form button[action=update]': {
            	click: this.validate
            }
        });
    },
    ...
    validate: function(button) {
        ...
        var store = Ext.getStore("MailedProcessingPieceToCustomers");
        if(form.form.isValid()){
            if(button.action === "save") {
            	...
            	//修改，找出修改哪一条
        		var record = Ext.ComponentQuery.query("mailed_processing_piece_to_customer_grid")[0].getSelectionModel().getSelection()[0];
    			record.set(values);
            }
            win.close();
        }
