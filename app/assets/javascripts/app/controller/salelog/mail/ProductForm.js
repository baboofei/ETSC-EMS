/**
 * 拆开后单独加载“新增/修改寄产品”视图用的controller
 */
Ext.define('EIM.controller.salelog.mail.ProductForm', {
...
//    }],
    ...
    init: function() {
        ...
        me.control({
            'mail_product_form combo[name=customer_id]': {
                render: this.loadCustomer,
                select: this.enableExpressButton
            },
            'mail_product_form combo[name=express_id]': {
                select: this.enableExpressButton
            },
            'mail_product_form combo[name=our_company_id]': {
                select: this.enableExpressButtonAndRadio
            },
            'mail_product_form print_express_sheet_button': {
                click: this.printExpressSheet
            },
            'mail_product_form button[action=save]': {
                click: this.validate
            },
            'mail_product_form button[action=update]': {
            	click: this.validate
            }
        });
    },
    ...
    validate: function(button) {
        ...
        var store = Ext.getStore("MailedProducts");
        if(form.form.isValid()){
            if(button.action === "save") {
            	...
            	//修改，找出修改哪一条
        		var record = Ext.ComponentQuery.query("mailed_product_grid")[0].getSelectionModel().getSelection()[0];
    			record.set(values);
            }
            win.close();
        }
