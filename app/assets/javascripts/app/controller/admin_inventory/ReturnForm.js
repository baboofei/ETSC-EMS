 ...
 */
Ext.define('EIM.controller.admin_inventory.ReturnForm', {
...
//        'ComboUser'
    ...
    init: function() {
        ...
        me.control({
            'admin_inventory_return_form [name=outstock_numbers]': {
                select: this.validateNumberByNumber
            },
            'admin_inventory_return_form button[action=save]': {
                click: this.submit
            }
        });
    },
    ...
    validateNumberByNumber: function(combo, records) {
        ...
        var grid = Ext.ComponentQuery.query('admin_inventory_return_query_grid')[0];
        ...
        if(records.length === expected_quantity) {
            ...
            combo.invalidMsg = '需要归还的数量为 ' + expected_quantity + '，请检查输入数量！';
            combo.validate();
        }
    },
            ...
            form.submit({
                ...
                success: function(the_form, action) {
                    ...
                    Ext.getStore('GridReturnQueryAdminInventories').load({
                        scope: this,
                        callback: function() {
                            if(Ext.getStore('GridReturnQueryAdminInventories').count() === 0) {
                                Ext.ComponentQuery.query('admin_inventory_return_query_form')[0].close();
                            }
                        }
                    });
                },
