                    ...
                    if i_filter['field'].include?("^")
                        ...
                        field_array.each do |field|
                            ...
                            if link_format.size == 2
                                #长度为2，对应上面第1种情况
                                #查出会有哪些态
                                existed_morph = self.where(true).map{|p| p["#{first_table_name}_type"]}.uniq.reject{|p| p.blank?}
                                #每一个和后面的“字段”搭配
                        ...
                        if i_filter['field'].split("|").size > 1
                            ...
                            if i_filter['field'].split(">").size > 1
                                ...
                                include_table_hash_array = []

                                probably_part_array.each_with_index do |probably, index|
                                    ...
                                    case join_table_queue_array.size
                                        ...
                                        when 2
                                            search_table_array << join_table_queue_array[0].underscore.pluralize
                                            search_model_array << join_table_queue_array[0]
                                            search_field_array << join_table_queue_array[1]
                                            collect_id_array << "id"

                                            include_table_hash_array << join_reflection_queue_array[0..-2].to_link_table_hash
                                        else
                                            search_table_array << join_table_queue_array[-2].underscore.pluralize
                                            search_model_array << join_table_queue_array[-2]
                                            search_field_array << join_table_queue_array[-1]
                                            collect_id_array << "id"
                                            #binding.pry
                                            include_table_hash_array << join_reflection_queue_array[0..-2].to_link_table_hash
                                    ...
                                    #        #        CustomerUnit.where("customer_unit_aliases.unit_alias like '%a%'").includes(:unit_aliases)
                                    #        #在Vendor里查vendor_unit的name，是这样的：["vendor_unit", "name"]
                                    #        #        Vendor.where("vendor_units.name like '%a%'").includes(:vendor_unit)
                                    #
                                    ...
                                    #        #20140804↓
                                    #        #打脸了，真大于3层了：
                                    #        #加了customer_unit_addr之后，在customer表过滤“单位名称”列：customer_unit_addr => customer_unit => customer_unit_alias => unit_alias
                                    #        #binding.pry
                                    ...
                                    #
                                    #        #以customer为例，目标是这样的：
                                    #        #Customer.where("customer_units.name like '%吉%'").includes(:customer_unit_addr => {:customer_unit => :unit_aliases})#其实这一行不用includes这么多，但多了也没坏处，毕竟都一样就少算一次
                                    #        #Customer.where("customer_unit_aliases.unit_alias like '%吉%'").includes(:customer_unit_addr => {:customer_unit => :unit_aliases})
                                    #        #两个结果都map到id，再加再uniq
                                    #
                                    ...
                                    #        search_model_array << model_name#join_table_queue_array[-2].singularize.camelize
                                    #        search_field_array << join_table_queue_array[-1]
                                ...
                                if search_table_array.uniq.size > 1
                                    #循环取出id
                                    id_array = []
                                    search_table_array.each_with_index do |p, index|
                                        #binding.pry
                                        id_array << result.where("#{p}.#{search_field_array[index]} like ?", "%#{i_filter['value']}%").includes(include_table_hash_array[index]).map(&:id)
                                    end

                                    id_array.flatten!.uniq!
                                    if id_array.size > 0
                                        result = result.where("#{model_name.underscore.pluralize}.id in (#{(["?"] * id_array.size).join(",")})", *id_array)
                                    else
                                    ...
                                    or_branch_array = []
                                    search_table_array.each_with_index do |p, index|
                                        or_branch_array << "#{p}.#{search_field_array[index]} like ?"
                                        result = result.includes(include_table_hash_array[index])
                                    end
                                    repeat_array = ["%#{i_filter['value']}%"] * probably_part_array.length
                                    result = result.where(or_branch_array.join(" or "), *repeat_array)
                                end
                            else
                                #无链式关联表，平级的or
...
#                                #    id_array += []
#                                #else
#                                p "#{search_model}.where('#{search_table}.#{search_field} like ?', '%#{i_filter['value']}%').includes(#{include_table_hash})"
#                                #p Customer.where("customer_units.name like '%吉%'").includes(:customer_unit_addr => {:customer_unit => :unit_aliases}).pluck(:id)
#                                id_array += search_model.where("#{search_table}.#{search_field} like ?", "%#{i_filter['value']}%").includes(include_table_hash).map{|p| p[collect_id]}
#                                #end
#                            end
#                            #p id_array
#                            id_array.uniq!
...
#                                result = result.where("id in (#{question_array})", *id_array).includes(include_table_hash)
#                            end
#                        else
#                            repeat_array = ["%#{i_filter['value']}%"] * probably_part_array.length
#                            result = result.where(probably_part_array.map{|p| "#{search_table_array[0].pluralize}.#{p} like ?"}.join(" or "), *repeat_array).includes(include_table_hash)
